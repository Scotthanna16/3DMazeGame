...
public class Map {
	 ...
	 */
	private void drawMap(MazePanel g, int px, int py, int walkStep,
		...
		g.setColor(ColorTheme.getColor(MazeColors.MAP_DEFAULT).toArgb());
	...
	private void drawVerticalLine(MazePanel g, boolean showMaze, int x, int y, int startX, int startY) {
		if (hasAVerticalWall(x, y) && 
			...
			g.setColor(seenWalls.hasWall(x, y, CardinalDirection.West) ? 
					ColorTheme.getColor(MazeColors.MAP_WALL_SEENBEFORE).toArgb() :
						ColorTheme.getColor(MazeColors.MAP_WALL_DEFAULT).toArgb());
	...
	private void drawCurrentLocation(MazePanel gc, int viewDX, int viewDY) {
		gc.setColor(ColorTheme.getColor(MazeColors.MAP_CURRENTLOCATION).toArgb());
	...
	private void drawSolution(MazePanel gc, int offsetX, int offsetY, int px, int py) {
		...
		gc.setColor(ColorTheme.getColor(MazeColors.MAP_SOLUTION).toArgb());
